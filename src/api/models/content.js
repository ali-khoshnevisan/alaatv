...
const APIAdresses = {
  ...
  customizeTimepoint: '/timepoint/customize-store',
  show: (id) => '/c/' + id,
...
export default class ContentAPI extends APIRepository {
  constructor () {
    ...
    this.CacheList = {
      ...
      customizeTimepoint: this.name + this.APIAdresses.customizeTimepoint,
  ...
  setCustomizeTimepoint (data = {}, cache) {
    return this.sendRequest({
      apiMethod: 'post',
      api: this.api,
      request: this.APIAdresses.customizeTimepoint,
      cacheKey: this.CacheList.customizeTimepoint,
      ...(cache && { cache }),
      resolveCallback: (response) => {
        return {
          timestamp: response.data
      ...
      data: this.getNormalizedSendData({
        content_id: null, // content Id
        comment: null, // Title for Timestamp
        time: null // time of Video for timestamp in seconds
      }, data)
    })
  }
