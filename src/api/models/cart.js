...
export default class CartAPI extends APIRepository {
  constructor () {
    ...
    this.APIAdresses = {
      ...
      getPaymentRedirectEncryptedLink: (device, paymentMethod, orderId, inInstalment, transactionId, rubikaUserToken) => {
        ...
        if (rubikaUserToken) {
          address += '&rubika_user_token=' + rubikaUserToken
        }
    ...
    this.CacheList = {
      ...
      getPaymentRedirectEncryptedLink: (device, paymentMethod, orderId, inInstalment, rubikaUserToken) => this.name + this.APIAdresses.getPaymentRedirectEncryptedLink(device, paymentMethod, orderId, inInstalment, rubikaUserToken),
  ...
  addToCart (data = {}, cache) {
    const payload = {
      ...
      seller: this.seller,
      ...(data.fixDiscountPercentage && { fixDiscountPercentage: data.fixDiscountPercentage }),
    ...
    const mergedData = this.getNormalizedSendData(
      {
        ...
        rubika_user_token: null // String
      }, data)

    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.getPaymentRedirectEncryptedLink(mergedData.device, mergedData.paymentMethod, mergedData.orderId, mergedData.inInstalment, mergedData.transactionId, mergedData.rubika_user_token),
      cacheKey: this.CacheList.getPaymentRedirectEncryptedLink(mergedData.device, mergedData.paymentMethod, mergedData.orderId, mergedData.inInstalment, mergedData.transactionId, mergedData.rubika_user_token),
      ...(cache !== undefined && { cache }),
      resolveCallback: (response) => {
