...
export default class ProductAPI extends APIRepository {
  constructor () {
    ...
    this.APIAdresses = {
      ...
      sampleContent: (id) => '/product/' + id + '/sample2',
  ...
  sampleContent (productId, cache) {
    return this.sendRequest({
      ...
      resolveCallback: (response) => {
        return {
          pamphlets: new ContentList(response.data.data.pamphlets),
          videos: new ContentList(response.data.data.videos),
          sample_photos: new ContentList(response.data.data.sample_photos)
        }
      },
      rejectCallback: (error) => {
        return error
