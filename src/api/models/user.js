...
import { TimepointList } from 'src/models/Timepoint.js'
...
export default class UserAPI extends APIRepository {
  constructor () {
    ...
    this.APIAdresses = {
      ...
      entekhabReshte: (eventId) => '/user/get/entekhab-reshte?event_id=' + eventId,
      bankAccounts: '/bank-accounts',
      mobileResend: '/mobile/resend',
      mobileVerify: '/mobile/verify',
      ordersById: (id) => '/user/' + id + '/orders',
      briefOrdersById: (id) => '/user/' + id + '/briefOrders',
      isPermittedToPurchase: (productId) => '/user/isPermittedToPurchase/' + productId,
      transactionById: (transactionId, userId) => '/check/transaction/' + transactionId + '/user/' + userId,
      getOrders: '/orders',
      orderStatus: '/payment/status',
      formData: '/megaroute/getUserFormData',
      showUser: '/getUserFor3a',
      eventResult: '/event-result',
      customizeTimepoints: '/user/contents/customize-timepoints',
      showEventResult: (eventId, userId) => '/event-result?event_id=' + eventId + '&user_id=' + userId,
      storeLastKonkur: 'event-result/store-last-konkur',
      getEntekhabReshteByUserId: (userId, eventId) => '/user/get/entekhab-reshte?event_id=' + eventId + '&user_id=' + userId,
      ...
      newsletterStatus: '/newsletter/show',
    ...
    this.CacheList = {
      ...
      verifyMoshavereh: this.name + this.APIAdresses.verifyMoshavereh,
      newsletterStatus: this.name + this.APIAdresses.newsletterStatus,
      bankAccounts: this.name + this.APIAdresses.bankAccounts,
      customizeTimepoints: this.name + this.APIAdresses.customizeTimepoints,
      byId: (id) => this.name + this.APIAdresses.byId(id),
      ordersById: (id) => this.name + this.APIAdresses.ordersById(id),
      briefOrdersById: (id) => this.name + this.APIAdresses.briefOrdersById(id),
      isPermittedToPurchase: (productId) => this.name + this.APIAdresses.isPermittedToPurchase(productId),
      transactionById: (transactionId, userId) => this.name + this.APIAdresses.transactionById(transactionId, userId),
      ...
      showEventResult: (eventId, userId) => this.name + this.APIAdresses.showEventResult(eventId, userId),
      entekhabReshte: (eventId) => this.name + this.APIAdresses.entekhabReshte(eventId),
      getEntekhabReshteByUserId: (userId, eventId) => this.name + this.APIAdresses.getEntekhabReshteByUserId(userId, eventId),
  ...
  getContentsWithCustomizeTimepoints (cache = { TTL: 1000 }) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.customizeTimepoints,
      cacheKey: this.CacheList.customizeTimepoints,
      ...(cache && { cache }),
      resolveCallback: (response) => {
        return new TimepointList(response.data.data)
      },
  ...
  checkTransactionById (data, cache = { TTL: 1000 }) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.transactionById(data.transactionId, data.userId),
      cacheKey: this.CacheList.transactionById(data.transactionId, data.userId),
      ...(cache && { cache }),
      resolveCallback: (response) => {
  ...
  ordersById (data = {}) {
    return this.sendRequest({
      ...
      ...(data?.cache && { cache: data.cache }),
      data: this.getNormalizedSendData({
        product_ids: null, // Array of Number
        is_installment_late: null // 1 means is late and 0 means not late
      }, data.params),
      resolveCallback: (response) => {
        return new OrderList(response.data.data)
      },
      rejectCallback: (error) => {
        return error
      }
  ...
  briefOrdersById (data = {}) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.briefOrdersById(data.data.userId),
      cacheKey: this.CacheList.briefOrdersById(data.data.userId),
      ...(data?.cache && { cache: data.cache }),
      data: this.getNormalizedSendData({
        product_ids: null, // Array of Number
        is_installment_late: null // 1 means is late and 0 means not late
      ...
      resolveCallback: (response) => {
        return {
          cartItemList: new OrderList(response.data.data)
        }
      },
      rejectCallback: (error) => {
        return error
  ...
  getUserData (userId, cache = { TTL: 1000 }) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.byId(userId),
      cacheKey: this.CacheList.byId(userId),
      ...(cache && { cache }),
      resolveCallback: (response) => {
        return new User(response.data.data)
      },
  ...
  eventResult (data = {}) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.showEventResult(data.event_id, data.user_id),
      cacheKey: this.CacheList.showEventResult(data.event_id, data.user_id),
      ...(data.cache && { cache: data.cache }),
      resolveCallback: (response) => {
        return new EventResult(response.data.data[0])
      },
      rejectCallback: (error) => {
        return error
      }
  ...
  getEntekhabReshteByUserId (userId, eventId, cache = 1000) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.getEntekhabReshteByUserId(userId, eventId),
      cacheKey: this.CacheList.getEntekhabReshteByUserId(userId, eventId),
      ...(cache && { cache }),
      resolveCallback: (response) => {
  ...
  getEntekhabReshte (data, cache = 1000) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.entekhabReshte(data.eventId),
      cacheKey: this.CacheList.entekhabReshte(data.eventId),
      ...(cache && { cache }),
      resolveCallback: (response) => {
  ...
  verifyMoshavereh (data, cache = { TTL: 1000 }) {
    return this.sendRequest({
      apiMethod: 'post',
      api: this.api,
      request: this.APIAdresses.verifyMoshavereh,
      cacheKey: this.CacheList.verifyMoshavereh,
      data: this.getNormalizedSendData({
        code: '' // String
      }, data),
      resolveCallback: (response) => {
  ...
  newsletterStatus (data, cache = { TTL: 1000 }) {
    return this.sendRequest({
      apiMethod: 'get',
      api: this.api,
      request: this.APIAdresses.newsletterStatus,
      cacheKey: this.CacheList.newsletterStatus,
      ...(cache !== undefined && { cache }),
      data: this.getNormalizedSendData({
        mobile: '', // String
