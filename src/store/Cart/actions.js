...
export function addToCart (context, newProductData) {
  ...
  return new Promise((resolve, reject) => {
    const payload = {
      ...
      showNotify: typeof newProductData.showNotify !== 'undefined' ? newProductData.showNotify : true,
      ...(newProductData.product.price.fixDiscountPercentage && { fixDiscountPercentage: newProductData.product.price.fixDiscountPercentage }),
    ...
    if (isUserLogin) {
      APIGateway.cart.addToCart(payload)
        .then((response) => {
          updateCart(payload)
          if (!payload.has_instalment_option && payload.showNotify) {
            showNotify()
          }
...
export function paymentCheckout (context, payload) {
  const { paymentMethod, transactionId } = payload
  return new Promise((resolve, reject) => {
    APIGateway.cart.getPaymentRedirectEncryptedLink({ device: 'web', paymentMethod, transactionId })
...
export function removeItemFromCart (context, data) {
  const payload = {
    product: data.product, // new Product()
    showNotify: typeof data.showNotify !== 'undefined' ? data.showNotify : true // Boolean
  }
  const setCartLoading = (loadingState) => {
    const cart = context.getters.cart
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isUserLogin) {
      setCartLoading(true)
      APIGateway.cart.removeFromCartByProductId(payload.product.id)
        .then((response) => {
          removeByProductId(payload.product.id)
          // removeByOrderProductId(orderProductId)
          pushAEEEvent(payload.product)
          if (payload.showNotify) {
            showNotify()
          }
          setCartLoading(false)
          resolve(response)
  ...
  return new Promise((resolve, reject) => {
    if (isUserLogin) {
      cart.items.list.forEach((item) => {
        // TODO => very bad code
        context
          .dispatch('removeItemFromCart', { product: item.id })
          .then((response) => {
