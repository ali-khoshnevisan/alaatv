        ...
        <template v-else-if="cart.count > 0">
          <div v-if="isUserLogin">
            ...
            <q-card class="invoice-cart">
              ...
              <q-card-section v-if="!isRubikaUser"
                ...
                <div v-if="localOptions.hasDiscountPercent && !localOptions.dense"
                  ...
                  <div v-if="cart.coupon.type === 'upToAmount'"
                       class="text-red q-mb-sm">
                    با استفاده از این کد، تخفیف خود محصولات در مبلغ نهایی درنظر گرفته نمیشود.
                  </div>
                ...
                <div v-if="localOptions.hasGiftcard && !localOptions.dense && !isEwanoUser"
              ...
              <q-card-section class="payment-section invoice-cart-section">
                <div v-if="localOptions.hasVatlPrice"
                     class="vat">
                  <div class="title">{{localOptions.vatPrice}}</div>
                  <div class="price">
                    {{vatPrice}}
                    <span class="iran-money-unit">تومان</span>
                  </div>
...
import mixinRubika from 'components/Widgets/Rubika/mixinRubika.js'
...
export default {
  ...
  data () {
    return {
      ...
      defaultOptions: {
        ...
        hasFinalPrice: true,
        vatPrice: 'مالیات بر ارزش افزوده',
        hasVatlPrice: true,
  ...
  computed: {
    ...
    vatPrice () {
      return this.getPriceFormat('vat') ? this.getPriceFormat('vat') : 0
    },
  ...
  mounted () {
    this.mounted = true
    if (this.getRubikaToken()) {
      this.checkIsRubikaUser()
        .then(() => {
          if (this.isUserLogin) {
            this.setUserInfo()
          }
  ...
  methods: {
    ...
    getGateways () {
      ...
      if (this.isRubikaUser) {
        this.gateways = new GatewayList([{
          ...
          photo: 'https://nodes.alaatv.com/upload/alaaPages/2024-09/Rubika_Icon1726408954.png'
        }])
        this.selectedBank = this.gateways.list[0].name
        return
      }
      if (this.isShadUser) {
        this.gateways = new GatewayList([{
          ...
          order: 1,
          // photo: 'https://nodes.alaatv.com/upload/alaaPages/2023-11/ewano_logo.png'
          photo: 'https://nodes.alaatv.com/upload/alaaPages/2025-06/shadLogo1748981621.png'
        }])
        this.selectedBank = this.gateways.list[0].name
        return
      }
    ...
    getRubikaGatewayUrl () {
      const rubikaUserToken = this.getRubikaToken()
      APIGateway.cart.getPaymentRedirectEncryptedLink({
        device: 'web',
        paymentMethod: 'rubika',
        rubika_user_token: rubikaUserToken
      })
        .then(url => {
