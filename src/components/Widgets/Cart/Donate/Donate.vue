<template>
  <div class="donate bg-white"
       :class="{'hidden': isEwanoUser || isRubikaUser}">
    <p>کمک مالی به آلاء</p>
    <q-separator />
    <div class="row  q-col-gutter-x-xl q-my-md text-center reverse-wrap">
...
export default {
  ...
  computed: {
    ...
    donateImage () {
      const sadImage = 'https://nodes.alaatv.com/upload/alaaPages/2024-09/Artboard40copy1725971171.png'
      const happyImage = 'https://nodes.alaatv.com/upload/alaaPages/2024-09/Artboard40copy21725971204.png'
  ...
  methods: {
    ...
    removeDonateFromCart () {
      this.$store.dispatch('Cart/removeItemFromCart', { product: new Product({ id: this.donateProductId }), showNotify: false })
        .then(() => {
          this.$store.dispatch('Cart/reviewCart')
          // this.$emit('cart-review')
        })
        .catch(() => {})
    },
    addDonateToCart () {
      this.$store.dispatch('Cart/addToCart', { product: new Product({ id: this.donateProductId }), showNotify: false })
        .then(() => {
          this.$store.dispatch('Cart/reviewCart')
          // this.$emit('cart-review')
        })
