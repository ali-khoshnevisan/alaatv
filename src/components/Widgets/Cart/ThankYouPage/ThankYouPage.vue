...
export default {
  ...
  methods: {
    ...
    updateUserData () {
      ...
      APIGateway.user.getUserData(userId)
        .then((user) => {
    ...
    onLoadPage () {
      ...
      APIGateway.cart.getorderWithTransaction(this.orderId)
        .then((order) => {
          this.loading = false
          order.successful_transactions.forEach(item => {
            if (item.paymentmethod.name === 'wallet') {
              this.updateUserData()
            }
          })

          if (order.paymentstatus.id === 3 || order.paymentstatus.id === 2) {
            this.hasPaid = true
