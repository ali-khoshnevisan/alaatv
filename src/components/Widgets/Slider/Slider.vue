<template>
  <q-carousel ref="SliderCarousel"
    ...
    <q-carousel-slide v-for="(slide, index) in options.list"
      ...
      <div :class="{'none-pointer': !slide.hasAction}"
           class="slider-parent">
...
export default {
  ...
  computed: {
    parentComponent () {
      if (this.selectedSlide.action.route && this.selectedSlide.action.name === 'link') {
        if (this.isExternal(this.selectedSlide.action.route)) {
          return 'a'
        } else {
          return 'router-link'
        }
  ...
  methods: {
    ...
    takeAction (slide) {
      ...
      if (!slide.hasAction) {
        return null
      } else if (slide.action.name && slide.action.name === 'scroll') {
        this.scrollToElement(slide.action.scrollTo)
      } else if (slide.action.name && slide.action.name === 'link') {
        this.redirectRoute(slide.action.route)
      } else if (slide.action.name && slide.action.name === 'event') {
        this.$bus.emit(slide.action.eventName, slide.action.eventArgs)
      }
      if (this.parentComponent === 'a') {
        openURL(slide.action.route)
      } else if (this.parentComponent === 'router-link') {
        this.$router.push(slide.action.route)
      }
    },
    hasVideo (slide) {
