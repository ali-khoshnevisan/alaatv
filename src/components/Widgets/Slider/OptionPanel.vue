    ...
    <template #main-tab>
      ...
      <q-table dir="rtl"
        ...
        <template v-slot:body="props">
          <q-tr :props="props"
                draggable="true"
                @dragstart="onDragStart($event, props.row, props.row.name)"
                @dragover="onDragOver"
                @dragleave="onDragLeave"
                @drop="onDrop($event, props.row.name - 1)">
            <q-td key="name"
                  :props="props">
              {{ props.row.name }}
            </q-td>
      ...
      <q-card-section>
        <div class="row q-col-gutter-md">
          ...
          <div v-if="localOptions.list[selectedBannerIndex].hasAction"
            ...
            <action-options v-model:action="localOptions.list[selectedBannerIndex].action.name"
                            v-model:scroll-to="localOptions.list[selectedBannerIndex].action.scrollTo"
                            v-model:link="localOptions.list[selectedBannerIndex].action.route"
                            v-model:event-name="localOptions.list[selectedBannerIndex].action.eventName"
                            v-model:event-args="localOptions.list[selectedBannerIndex].action.eventArgs" />
          </div>
          <div class="col-12">
...
import actionOptions from 'components/WidgetComponents/ActionOptions/ActionOptions.vue'
...
export default defineComponent({
  ...
  methods: {
    ...
    onDragStart (event, banner, bannerName) {
      const bannerIndex = this.rows.findIndex(row => row.name === bannerName)
      event.dataTransfer.dropEffect = 'move'
      event.dataTransfer.setData('value', JSON.stringify({ banner, bannerIndex }))
    ...
    onDrop (event, newIndex, parent) {
      ...
      const bannerOldIndex = value.bannerIndex
      const bannerNewIndex = newIndex
      if (this.localDraggable) {
        this.updatePosition(this.localOptions.list, bannerOldIndex, bannerNewIndex)
      } else {
        this.addToIndex(this.localOptions.list, banner, bannerNewIndex)
      }
