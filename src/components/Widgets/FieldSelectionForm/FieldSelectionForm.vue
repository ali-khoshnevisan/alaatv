    ...
    <q-tab-panels v-if="!loading"
      ...
      <q-tab-panel name="RegisterKonkurFieldSelectionProducts">
        <register-konkur-field-selection-products v-model:order-id="orderId"
                                                  ...
                                                  v-model:limited-product-enable="localOptions.limitedProductEnable"
...
export default {
  ...
  data () {
    return {
      ...
      defaultOptions: {
        ...
        product5Id: 1243,
        product5Enable: true,
        limitedProductEnable: 1241,
        ticketDepartmentId: 29
      }
    }
  },
  mounted () {
  ...
  methods: {
    ...
    checkHasPurchased (resolve, reject) {
      this.getTicketDepartemant()
        .then((ticketList) => {
          this.checkKonkurResultFormData(ticketList, resolve, reject)
        })
        .catch(() => {
          reject()
        })
    },
    checkKonkurResultFormData (ticketList, resolve, reject) {
      if (ticketList.list.length > 0) {
    ...
    getTicketDepartemant () {
      return new Promise((resolve, reject) => {
        APIGateway.ticket.index({ department_id: [this.localOptions.ticketDepartmentId] })
          .then(ticketList => {
            resolve(ticketList.list)
          })
          .catch(() => {
            reject()
          })
    ...
    getFieldSelectionFormData () {
      return new Promise((resolve, reject) => {
        APIGateway.user.getEntekhabReshte({ eventId: this.localOptions.eventId })
          .then((eventekhbReshte) => {
