  ...
  <div class="RegisterKonkurFieldSelectionProducts">
    <template v-if="!loading && !isSelectionFieldProductsEnable">
      <q-banner class="text-center q-mb-md">
        ...
        <br>
        متاسفانه
        ظرفیت انتخاب رشته کنکور 1403 به پایان رسیده است.
      </q-banner>
    </template>
    <template v-else>
      ...
      <div class="content-text q-mb-xl">
        بر اساس نیازت میتونی یکی از طرح ها زیر رو انتخاب کنی؛ البته اگه راه ابریشمی . بودی پیشنهاد آلاء اینه که از طرح های بررسی انتخاب رشته استفاده کنی.
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      products: [
        {
          value: 0,
          label: 'انتخاب رشته ویژه کاربران راه ابریشم 2',
          items: [
            {
              enable: true,
              caption: '',
        ...
        {
          value: 1,
          label: 'انتخاب رشته ویژه کاربران راه ابریشم 1',
          items: [
            {
              enable: true,
              caption: '',
  ...
  computed: {
    ...
    isSelectionFieldProductsEnable () {
      const disableProduct = this.productObjectList.list.find(p => !p.enable)
      return this.productObjectList.list.length > 0 && !disableProduct
    }
  },
  watch: {
    limitedProductEnable () {
      this.loadLimitedProduct()
    }
  },
  mounted () {
    this.setProductIds()
  ...
  methods: {
    setProductIds () {
      ...
      this.products[2].items[0].id = this.product3Id
      this.products[2].items[1].id = this.product4Id
      this.products[2].items[2].id = this.product5Id
      this.selectedProduct = this.product1Id
    },
    loadProductPrice () {
      this.products[0].items[0].value = this.getPriceString(this.getProductById(this.product1Id))
    ...
    loadLimitedProduct () {
      this.products[2].items.forEach(item => {
        item.enable = item.id === this.limitedProductEnable
        this.products[2].value = this.limitedProductEnable
      })
    },
    ...
    getPriceString (product) {
      const finalPrice = product.price.final + product.price.vat
      return finalPrice.toLocaleString('fa')
    },
    getProducts () {
      this.loading = true
