    ...
    <q-card class="ShowLiveClassesLink">
      ...
      <q-card-section>
        ...
        <template v-else-if="products.list.length > 0">
          ...
          <q-separator v-if="isAnyProductLive"
                       class="q-my-lg" />
          <div class="row q-col-gutter-lg">
            <div v-for="product in products.list.filter(product => !product.is_live)"
                 :key="product.id"
  ...
  <q-dialog v-model="infoDialog"
    ...
    <q-card>
      ...
      <q-card-section class="q-pt-none">
        <q-input v-model="temp_first_name"
                 dense
                 autofocus
                 :loading="user.loading || liveLinkLoading"
                 label="نام" />
        <q-input v-model="temp_last_name"
                 dense
                 autofocus
                 :loading="user.loading || liveLinkLoading"
                 label="نام خانوادگی" />
...
export default {
  ...
  computed: {
    isExistLiveProduct () {
      return this.products.list.filter(product => product.is_live).length > 0
    },
    isAnyProductLive () {
      return this.products.list.some(product => product.is_live) && this.products.list.some(product => !product.is_live)
    }
  },
  mounted () {
  ...
  methods: {
    ...
    updateFullname () {
      ...
      APIGateway.user.updateProfile(this.user.id, { first_name: this.temp_first_name, last_name: this.temp_last_name })
        .then(() => {
