...
export default {
        ...
        },
        {
          ...
          value: [
            ...
            },
            {
              ...
              label: 'تاریخ تولد',
              outlined: true,
              col: 'col-xs-6 q-mb-md required-date',
              value: null
            },
            {
              type: 'select',
  ...
  mounted () {
    if (this.isShadUser) {
      this.inputs = this.shadUsersInputs
      this.localInputs = this.shadUsersInputs
    }
    this.getFormData()
  ...
  methods: {
    ...
    submit () {
      const g = this.inputs.filter(x => x.name === 'personalInfo')[0].value.filter(x => x.name === 'gender')[0].value
      if (g === null) {
        this.$q.notify({
