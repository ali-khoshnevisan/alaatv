<template>
  <div class="triple-title-set-study-plan">
    <div class="row">
      ...
      <div class="col-md-6 col-12  action-btns">
        <q-btn flat
               label="تنظیمات برنامه مطالعاتی"
               icon="ph:nut"
               class="q-btn-md"
      ...
      <div class="col-12 calendar">
        <full-calendar v-if="studyPlanListLoaded"
                       ...
                       :educational-layers="event.config.educational_layers"
...
export default {
  ...
  methods: {
    ...
    afterGetData () {
      const major = FormBuilderAssist.getInputsByName(this.editInputs, 'grade_id').options.filter(grade => grade.title === this.currentStudyPlan.grade)
      FormBuilderAssist.setAttributeByName(this.editInputs, 'grade_id', 'value', major[0].id)
    },
    updatePlan () {
      this.loading = true
