<template>
  <q-item v-ripple
          clickable
          class="plan-item-box"
          @click="goToContent">
    <q-item-section>
      <q-item-label>{{content.title}}</q-item-label>
      <q-item-label v-if="content.set.title"
                    caption
                    lines="2">
        {{ content.set.title }}
      </q-item-label>
    </q-item-section>

    <q-item-section side>
      <q-btn v-if="content.isPamphlet()"
             color="primary"
             round
             unelevated
             :disable="!content.id"
             icon="download"
             class="size-xs"
             @click="downloadPdf(content)" />
      <q-btn v-if="content.isVideo()"
             color="primary"
             round
             unelevated
             :disable="!content.id"
             class="size-xs"
             icon="play_arrow"
             target="_blank"
             :to="{name:'UserPanel.Asset.TripleTitleSet.Content', params:{ productId: plan.product?.id, setId: content.set?.id, contentId: content?.id}}" />
    </q-item-section>
  </q-item>
</template>

<script>
import { Content } from 'src/models/Content.js'
import { Plan } from 'src/models/Plan'
import { APIGateway } from 'src/api/APIGateway'

export default {
  name: 'PlanContent',
  props: {
    plan: {
      type: Plan
    },
    content: {
      type: Content
    }
  },
  methods: {
    findEventNameFromId () {
      return new Promise((resolve, reject) => {
        APIGateway.events.index()
          .then((events) => {
            const eventName = events.list.find(event => event.id === Number(this.content.event_id)).name
            resolve(eventName)
          })
          .catch(() => {
            reject()
          })
      })
    },
    goToContent () {
      if (this.content.isPamphlet()) {
        this.downloadPdf(this.content)
        return
      }
      this.findEventNameFromId()
        .then((eventName) => {
          const routeData = this.$router.resolve({ name: 'UserPanel.Asset.TripleTitleSet.Content', params: { eventName, productId: this.plan.product?.id, setId: this.content.set?.id, contentId: this.content?.id } })
          window.open(routeData.href, '_blank')
        })
        .catch()
    },
    downloadPdf (content) {
      if (!content?.file?.pamphlet || !content?.file?.pamphlet[0] || !content?.file?.pamphlet[0].link) {
        this.$q.notify({
          type: 'negative',
          message: 'مشکلی در دانلود فایل pdf این محتوا رخ داده است.'
        })
        return
      }

      window.open(content.file.pamphlet[0].link, '_blank')
    }
  }
}
</script>

<style lang="scss" scoped>
.plan-item-box{
}
</style>
