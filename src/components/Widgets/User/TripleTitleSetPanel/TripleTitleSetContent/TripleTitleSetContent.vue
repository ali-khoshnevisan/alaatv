    ...
    <div class="row q-col-gutter-x-md">
      <div class="video-box-col col-12 col-xs-12 col-sm-12 col-lg-8">
        <video-box :set="selectedSet"
                   ...
                   :study-event-id="studyPlanInfo.id"
...
import studyPlanSelectionDialog from 'components/DashboardTripleTitleSet/Dashboard/StudyPlanSelectionDialog.vue'
...
export default {
  ...
  watch: {
    'watchingContent.completelyWatched': function (val) {
  ...
  methods: {
    ...
    videoIsWatched (secondsWatched) {
      return new Promise((resolve, reject) => {
        ...
        let sendData
        if (this.studyPlanInfo.id) {
          sendData = {
            completely_watched: 1,
            studyevent_id: this.studyPlanInfo.id,
        ...
        APIGateway.content.setVideoWatched(sendData)
          .then(() => {
            if (sendData.completely_watched) {
              this.isVideoWatched = true
              this.watchingContent.completelyWatched = true
              this.storeSelectedSet(this.$route.params.setId)
            }
            resolve()
          })
    ...
    updateTime (time) {
      this.currentVideoTime = time.currentTime
    },
    updateVideoStatus (data) {
      const hasWatch = data || this.watchingContent.completelyWatched
    ...
    setVideoStatusToUnwatched () {
      ...
      APIGateway.content.setVideoUnWatched({
        ...
        .then(() => {
          this.isVideoWatched = false
          this.watchingContent.completelyWatched = false
          this.storeSelectedSet(this.$route.params.setId)
        })
        .catch(() => {
        })
    ...
    storeSelectedTopic () {
      ...
      const splitted = this.selectedSet.short_title.split('-')
      let topic
      if (splitted.length === 4) {
        topic = splitted[1].trim() + '-' + splitted[2].trim()
      } else {
        topic = splitted[1].trim()
      }
    ...
    getProduct () {
      this.$store.dispatch('TripleTitleSet/getSelectedProduct', { productId: this.$route.params.productId, eventId: this.event.id })
    },
    goToNextSet () {
