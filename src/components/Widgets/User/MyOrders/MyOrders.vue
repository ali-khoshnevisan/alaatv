  ...
  <div>
    ...
    <template v-else>
      ...
      <q-dialog v-model="detailsDialog"
        ...
        <order-details-dialog :loading="detailsDialogLoading"
                              :is-admin="isAdmin"
                              :order="currentOrder"
...
export default {
  ...
  methods: {
    loadApi () {
      this.getEntityApi = APIGateway.user.APIAdresses.briefOrdersById(this.computedUserId)
    },
    ...
    showDetailsDialog (rowData) {
      ...
      this.getOrderData(rowData.id)
    },
    getOrderData (orderId) {
      APIGateway.order.getOrderById(orderId)
        .then(order => {
          this.detailsDialogLoading = false
          if (this.isAdmin) {
            this.$emit('showDetails', new Order(order))
          }
