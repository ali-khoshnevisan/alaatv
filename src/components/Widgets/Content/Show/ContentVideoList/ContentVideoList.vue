            ...
            <div v-for="(content,index) in set.contents.list"
              ...
              <div class="content q-pt-md q-px-sm"
                ...
                <div class="row content-show">
                  <div v-if="content && content.type"
                       class="col-1 q-mr-sm">
                    <router-link v-if="content.type === 8"
                  ...
                  <div class="col-10">
                    <router-link v-if="content && content.type && content.type === 8"
                                 :to="{name:'Public.Content.Show', params: {id: content.id}}">
...
export default {
  ...
  methods: {
    ...
    findEventNameFromId () {
      return new Promise((resolve, reject) => {
        APIGateway.events.index()
          .then((events) => {
            const event = events.list.find(event => event.id === Number(this.content.event_id))
            this.content.loading = false
            resolve(event.name)
          })
          .catch(() => {
    ...
    goToSet () {
      ...
      if (this.content.is_free || !this.content.event_id) {
        this.$router.push({ name: 'Public.Set.Show', params: { id: this.set.id } })
      } else {
        this.content.loading = true
        this.findEventNameFromId()
          .then((eventName) => {
            const setTitleListForExpand = this.set.short_title.split('-').splice(1).join('-')
            this.$router.push({ name: 'UserPanel.Asset.TripleTitleSet.ProductPage', params: { eventName, productId: this.content.related_product.id }, query: { expandTitle: setTitleListForExpand } })
          })
          .catch(() => {})
      }
    },
