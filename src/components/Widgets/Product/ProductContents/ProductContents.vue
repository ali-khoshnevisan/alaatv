  ...
  <div v-if="set.contents || set.pamphlets_count"
    ...
    <q-card class="previewSetsOfProduct custom-card q-mb-md">
      ...
      <div v-if="setOptions.length > 0"
        ...
        <!--        <div class="text-white text-subtitle1 text-weight-bolder title">-->
        <!--          انتخاب دوره-->
        <!--        </div>-->
...
export default {
  ...
  methods: {
    ...
    getSet (id) {
      ...
      this.$apiGateway.set.show(id)
        .then(set => {
          ...
          this.videos = []
          this.pamphlets = []
          this.contents = new ContentList(set.contents)
          this.setContents(set.contents)
        })
        .catch(() => {
          this.set.loading = false
        })
    ...
    setTab () {
      if (this.contents.list.some(content => content.isVideo())) {
        this.tab = 'videos'
      } else {
        this.tab = 'pamphlets'
      }
