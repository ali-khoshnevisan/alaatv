...
export default {
  ...
  mounted () {
    if (this.$route.path === '/landing/R100k') {
      this.checkSignup()
    }
    // this.$bus.on('getEventProducts', (data) => this.getProducts(data))
  },
  methods: {
    checkSignup () {
    ...
    getProducts (data) {
      if (!this.localOptions.getProductsByUser) {
        return
      }
      this.loading = true
      APIGateway.events.getEventsProducts({
        ...
        .then((productList) => {
          this.loading = false
          this.clonedData = this.optionsWithObjectProduct
          this.addProducts(this.clonedData, productList.list)
        })
        .catch(() => {
          this.loading = false
        })
