<template>
  ...
  <div v-else
    ...
    <div v-if="paymentMode === 'installment' && hasInstallment && !isEwanoUser && !isRubikaUser"
  ...
  <div v-if="showResponsive"
    ...
    <div v-else
      ...
      <div class="mobile-discount">
        <q-badge v-if="computedProductPrice.discountInPercent() > 0"
                 color="negative"
                 size="xs"
                 text-color="white"
                 :label="'%' + computedProductPrice.discountInPercent()" />
      ...
      <div class="mobile-price-calculation">
        <div v-if="computedProductPrice.discountInPercent() > 0"
             class="mobile-price-calculation-base">
          {{ computedProductPrice.toman('base', null) }}
        </div>
    ...
    <div class="mobile-price-action">
      <q-btn color="primary"
             ...
             :class="{'has-installment': hasInstallment && !isEwanoUser && !isRubikaUser}"
             size="md"
             :label="hasInstallment ? 'ثبت نام نقدی' : 'ثبت نام'"
             @click="paymentAction('cash')" />
      <q-btn v-if="hasInstallment && !isEwanoUser && !isRubikaUser"
             color="accent"
             class="mobile-action-btn"
