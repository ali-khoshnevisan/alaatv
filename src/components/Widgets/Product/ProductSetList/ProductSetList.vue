...
import { mixinWidget, mixinCapacitorDownloadFile } from 'src/mixin/Mixins.js'
...
export default {
  ...
  mixins: [mixinWidget, mixinCapacitorDownloadFile],
  emits: ['updateSetList'],
  data () {
    return {
      setLoading: false,
    ...
    setSections () {
      ...
      const getSectionTitle = (shortTitle) => {
        if (shortTitle.split('-').length > 0 && shortTitle.split('-')[0] && shortTitle.split('-')[1]) {
          if (shortTitle.split('-').length === 4) {
            return shortTitle.split('-')[0] + ' - ' + shortTitle.split('-')[1] + ' - ' + shortTitle.split('-')[2]
          }
