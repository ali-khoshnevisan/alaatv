    ...
    <div v-if="isDesktop"
      ...
      <div class="menu-items">
        <q-list class="menu-items-list">
          <q-item v-for="(item, index) in menuItems.filter(menuItem => menuItem.visible)"
                  :key="index"
            ...
            <q-icon class="icon"
                    :class="[ $route.name===item.routeName ? 'activate, ph-fill' :'']"
                    :name="item.icon" />
          </q-item>
        </q-list>
      </div>
...
export default {
  ...
  methods: {
    ...
    updateMenuItemsFromEventInfo () {
      ...
      if (this.event.config.showDashboardIfHasPurchased || this.event.config.showStudyPlanIfHasPurchased) {
        this.checkHasPurchased()
          .then(() => {
            this.updateMenuItemVisibility('UserPanel.Asset.TripleTitleSet.Dashboard', this.event.config.showDashboardIfHasPurchased ? this.event.config.showDashboardIfHasPurchased : this.event.config.showDashboard)
            this.updateMenuItemVisibility('UserPanel.Asset.TripleTitleSet.StudyPlan', this.event.config.showStudyPlanIfHasPurchased ? this.event.config.showStudyPlanIfHasPurchased : this.event.config.showStudyPlan)
          })
          .catch(() => {
            this.updateMenuItemVisibility('UserPanel.Asset.TripleTitleSet.Dashboard', this.event.config.showDashboard)
            this.updateMenuItemVisibility('UserPanel.Asset.TripleTitleSet.StudyPlan', this.event.config.showStudyPlan)
          })
        return
      }
      this.updateMenuItemVisibility('UserPanel.Asset.TripleTitleSet.Dashboard', this.event.config.showDashboard)
