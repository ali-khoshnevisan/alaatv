  ...
  <div class="video-box">
    <div v-if="content.id"
      ...
      <div class="video-wrapper">
        <content-video-player v-if="content.can_see?.toString() !== '0'"
                              :options="{
                                studyEventId,
                                paginate: false,
                                urlParam: 'contentId',
    ...
    <div v-if="content.id"
      ...
      <q-btn class="seen-btn"
             :color="content.completelyWatched ? 'secondary' : 'white'"
             :outline="!content.completelyWatched"
             :text-color="content.completelyWatched ? 'white' : 'grey-9'"
             :class="{ 'seen-video-btn': content.completelyWatched, 'video-btn': !content.completelyWatched }"
             :loading="content.loading"
             :label="!content.completelyWatched ? 'تکمیل جلسه' : 'تکمیل شده'"
             icon="ph:check"
             @click="clickSeenButton" />
      <div v-if="$q.screen.gt.xs"
           class="next-prev-btn">
        <q-btn label="جلسه قبل"
               flat
               icon="ph:caret-right"
               @click="goToPrevContent" />
        <q-btn label="جلسه بعد"
               flat
      ...
      <q-card-section class="flex items-center justify-around">
        <div v-for="(item, index) in content.file.video"
          ...
          <q-btn flat
                 square
                 icon="ph:download"
                 @click="downloadVideoContent(item)" />
        </div>
      </q-card-section>
    </q-card>
  </q-dialog>
...
import { mixinAuth, mixinCapacitorDownloadFile } from 'src/mixin/Mixins.js'
...
export default {
  ...
  mixins: [mixinAuth, mixinCapacitorDownloadFile],
  props: {
    studyEventId: {
      type: Number,
      default: null
  ...
  methods: {
    goToNextContent () {
      const currentContentIndex = this.set.contents.list.findIndex(content => content.id === this.content.id)
      const nextContentId = this.set.contents.list[currentContentIndex + 1].id
      const params = this.$route.params
      params.contentId = nextContentId
    ...
    downloadVideoContent (item) {
      if (this.content.can_see !== 0 && this.content.can_see !== 3) {
        if (this.isNative) {
          this.downloadPdfWithAxios(item.link + (item.link.includes('?') ? '' : '?') + 'download=1')
        } else {
          openURL(item.link + (item.link.includes('?') ? '' : '?') + 'download=1')
        }
      }
    },
