<template>
  <div ref="videoPlayerWrapper"
    ...
    <div v-show="useAdvertiseBanner && !isInVastMode && showAdvertisingBanner && !neverShowBanner"
         ref="adOverPlayer"
         class="advertise-banner-wrapper">
      <div class="advertise-banner-slot">
        <q-btn icon="close"
               square
               color="grey"
               class="size-xs close-btn"
               @click.stop="neverShowBanner = true" />
        <slot name="advertisingBanner" />
      </div>
    </div>

    <div v-if="!isInVastMode"
         ref="inWatchingBanner"
         class="advertise-banner-wrapper">
      <div class="advertise-banner-slot">
        <slot name="inWatchingBanner" />
      </div>
    </div>
...
export default {
  ...
  mounted () {
    this.mounted = true
    this.$bus.on('goToTimepoint', (time) => {
      this.player.pause()
      this.player.currentTime(time)
    })
  ...
  methods: {
    ...
    async initPlayer (withVast) {
      ...
      this.player.ready(() => {
        ...
        if (this.useAdvertiseBanner) {
          const overlay = this.$refs.adOverPlayer
          const videoContainer = this.player.el()
          videoContainer.appendChild(overlay)
        }
        // overlay.className = 'custom-overlay'
        // overlay.innerHTML = '<p>Overlay Text</p>'
        this.setPlayerBrand()
    ...
    reInitVideo (withVast) {
      ...
      const source = this.isPlayerSourceList() ? this.source.list : this.source
      this.player.src(source)
      // this.player.showTextTrack('subtitles')
      // this.player.addRemoteTextTrack({
      //   kind: 'subtitles',
      //   src: 'src/assets/example.vtt',
      //   srclang: 'en',
      //   label: 'English',
      //   default: true
      // })
...
.vPlayer {
  ...
  .video-js {
    ...
    .vjs-big-play-button {
      ...
      border-radius: 100%;
      background: v-bind('playButtonColor');
      border: 2px solid;
      border-color: v-bind('playButtonBorder');
