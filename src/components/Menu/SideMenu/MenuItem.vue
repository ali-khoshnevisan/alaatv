        ...
        <q-list class="list-expansion">
          <div v-for="(subItem , i) in item.children"
            ...
            <q-item v-else
                    ...
                    :class="{'active-route': isActive(subItem)}"
              ...
              <q-item-section class="list-section"
                              avatar>
                <q-icon :name="itemIcon(subItem)"
                        :class="{'active ph-fill': isActive(subItem)}"
                        size="30" />
              </q-item-section>
...
export default {
  ...
  methods: {
    itemIcon (item) {
      ...
      if (item.title === 'مشاوره') {
        return 'ph:headset'
      }
      return 'ph:book-open'
    },
    isActive (item) {
      if (this.selectedTopic.split('-').length === 2) {
        return (item.title === this.selectedTopic.split('-')[1]) || (item.title === this.clickedItem.title)
      }
    ...
    itemSelected (item) {
      ...
      this.$bus.emit('itemSelected', item)
...
.side-menu-main-layout {
  ...
  .q-list {
    ...
    &.side-menu-list {
      ...
      .active-route {
        background-color: rgba(38, 166, 153, 0.12);
