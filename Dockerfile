FROM node:20.12-alpine AS prebuild
...
ARG ALAA_MINIO_SERVER=https://nodes.gaal.ir
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=10s --retries=3 CMD [ "curl", "-f", "-I", "http://localhost:3000/login" ]
